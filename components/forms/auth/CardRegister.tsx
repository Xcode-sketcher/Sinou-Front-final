...
// /**
//  * Componente de Registro (CardRegister)
//  *
//  * Formulário completo de cadastro de novos usuários (cuidadores).
//  * Inclui validação em tempo real de senha, confirmação de senha,
...
// Interfaces para tipagem de erros da API
 ...
 * - Feedback visual de erros
 * - Integração com AuthContext para registro
 * - Tratamento específico de erro de email duplicado
 */
const CardRegister: React.FC = () => {
    const { register } = useAuth();
    const [formData, setFormData] = useState<RegisterFormData>({
    ...
    const handleChange = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        // Limpa erro geral ao editar qualquer campo
        if (errors.general) {
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        setIsLoading(true);
        setErrors({}); // Limpa todos os erros antes da tentativa
        ...
        try {
            ...
            if (error.response?.status === 400 && 
                ...
                setErrors({ 
                    email: 'Este e-mail já está cadastrado. Tente fazer login ou use outro e-mail.' 
                });
                ...
                setErrors({ 
                    general: errorMessage || 'Dados inválidos. Verifique as informações e tente novamente.' 
                });
                ...
                // Erro genérico
                setErrors({ 
                    general: 'Falha no cadastro. Tente novamente ou entre em contato com o suporte.' 
                });
            }
        } finally {
            setIsLoading(false);
                ...
                {errors.general && (
                    <div className="mb-6 p-4 rounded-lg bg-red-500/10 border border-red-500/50 flex items-start gap-3">
                        ...
                        <div className="flex-1">
                            <p className="text-sm text-red-400 font-medium">Erro no cadastro</p>
                            <p className="text-xs text-red-300 mt-1">{errors.general}</p>
                        </div>
                    </div>
                )}
